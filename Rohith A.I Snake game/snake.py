...
class Snake:
    ...
    def set_path(self):
        if self.score == SNAKE_MAX_LENGTH - 1 and self.apple.pos in get_neighbors(self.head.pos):
            ...
            print('Snake is about to win.. , Rohith')
            return winning_path
        ...
        print('Rohith no available path, snake in danger!')
    ...
    def update(self):
        ...
        if self.score == ROWS * ROWS - INITIAL_SNAKE_LENGTH:  # If snake wins the game
            ...
            print("Rohith , Snake won the game after {} moves"
        ...
        if self.hitting_self() or self.head.hitting_wall():
            print("Snake is dead, trying again Rohith..")
            self.is_dead = True
            self.reset()

        if self.moves_without_eating == MAX_MOVES_WITHOUT_EATING:
            self.is_dead = True
            print("Snake got stuck, trying again Rohith..")
            self.reset()
